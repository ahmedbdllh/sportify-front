...
export const PlayerSignUp = ({ onClose, onSwitchToPlayerSignIn }) => {
  ...
  const handleResendCode = async () => {
    setIsResending(true);
    setResendMessage("");    try {      const { response, data } = await AuthService.resendVerificationCode(userEmail, "Player");
  ...
  const handleVerifyCode = async () => {
    ...
    setIsVerifying(true);
    setVerificationError("");    try {      const { response, data } = await AuthService.verifyEmail(userEmail, verificationCode, "Player");
      if (!response.ok) {
        if (
          data.msg &&
          data.msg.toLowerCase().includes("phone number") &&
