...
const SessionManagement = () => {
  ...
  const fetchSessions = async () => {
    ...
    try {
      ...
      if (token) {
        const response = await axios.get("https://sportify-auth.onrender.com/api/auth/sessions", {
  ...
  const terminateSession = async (sessionId) => {
    try {
      const token = localStorage.getItem("token")
      const response = await axios.delete(`https://sportify-auth.onrender.com/api/auth/sessions/${sessionId}`, {
        headers: { "x-auth-token": token },
      })
  ...
  const terminateAllSessions = async () => {
    try {
      const token = localStorage.getItem("token")
      const response = await axios.delete("https://sportify-auth.onrender.com/api/auth/sessions", {
        headers: { "x-auth-token": token },
      })
