...
const SearchBar = ({ 
  ...
  useEffect(() => {
    if (!isInitialized && filters.length > 0) {
      ...
      console.log('SearchBar initialized with filters, but NOT triggering search');
    } else if (!isInitialized && filters.length === 0) {
      // Even with no filters, initialize with empty state
      setIsInitialized(true);
      console.log('SearchBar initialized without filters, NOT triggering search');
    }
  ...
  useEffect(() => {
    if (!isInitialized || !hasUserInteracted) {
      console.log('SearchBar useEffect: Not initialized or no user interaction yet');
      return;
    }
    
    console.log('SearchBar useEffect: User has interacted, setting up debounced search');
    const timeoutId = setTimeout(() => {
      console.log('SearchBar: Executing debounced search after user interaction');
      triggerSearch(searchTerm, sortBy, sortOrder, activeFilters);
    }, 500); // Increased debounce time to reduce requests
    
    return () => {
      console.log('SearchBar: Clearing debounce timeout');
